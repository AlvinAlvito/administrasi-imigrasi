...
<head>
    ...
    <style>
        ...
        .lampiran-header-wrap {
            ...
            height: 140px;
            padding-top: 96px;
            /* ✅ dorong isi turun tanpa collapse */
        }
        ...
        .lampiran-right {
            position: absolute;
            top: 8px;
            /* geser blok judul dari tepi atas */
            right: 0;
            text-align: right;
            line-height: 1.2;
        }
...
<body>
    ...
    <table class="border table-tight">
        <tr>
            ...
            <td class="border w-50"><span class="fw-bold">Pimpinan</span><br>{{ $spd->pimpinan->name ?? '-' }}
                ({{ $jabatanPimpinan }})</td>
        </tr>
        <tr>
            <td class="border"><span class="fw-bold">Tanggal
                    Berangkat</span><br>{{ tanggal_indo($spd->tanggal_berangkat) }}</td>
            <td class="border"><span class="fw-bold">Tanggal Kembali</span><br>{{ tanggal_indo($spd->tanggal_kembali) }}
            </td>
        </tr>
        <tr>
    ...
    <div class="lampiran-header-wrap">
        ...
        <div class="lampiran-right">
            ...
            <div class="l4">TENTANG PERJALANAN DINAS JABATAN DALAM NEGERI BAGI PEJABAT NEGARA,<br>PEGAWAI NEGERI DAN
                PEGAWAI TIDAK TETAP</div>
        </div>
    </div>
    ...
    @php
        $asal = $kotaInstansi; // Kota instansi (misal: Tanjungbalai)
        $tujuan = $spd->tujuan ?? '-'; // Tujuan dari DB
    @endphp
    ...
    <table class="border table-tight">
        ...
        <tr>
            ...
            {{-- Nama biaya pakai isi transportasi dari SPD --}}
            <td class="border">
                {{ $spd->alat_transportasi ?? 'Transportasi' }}
            </td>

            {{-- Total biaya transport tetap dari rincian biaya --}}
            <td class="border">
                {{ rupiah($transport) }}
            </td>

            {{-- Keterangan juga pakai isi transportasi, biar nggak statis --}}
            <td class="border">
                {{ $spd->alat_transportasi
                    ? 'Biaya ' . $spd->alat_transportasi . ' perjalanan dinas'
